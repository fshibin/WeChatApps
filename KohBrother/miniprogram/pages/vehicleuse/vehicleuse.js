        ...
        success: res => {
          ...
          if (res.result.stats.updated == 1) {
            that.setData({
              ...
              lastUsedJobAddr: that.data.jobAddr,
            })
            ...
            getApp().globalData.lastUsedJobAddr = that.data.jobAddr;
            that.addSURecord();
          } else {
  ...
  addSURecord : function() {
    ...
    db.collection('siteuse').add({
      ...
      success: function(res) {
        getApp().showSuccess('车辆使用记录添加成功！\nVehicle use record is added successfully!');
      },
      fail: function(res) {
        getApp().showError('无法添加车辆使用记录！\nUnable to insert vehicle use record!');
      }
    })
  },
