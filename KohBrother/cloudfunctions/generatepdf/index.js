...
exports.main = async (event, context) => {
  ...
  // print headers
  doc.setFontSize(40);
  doc.text('Koh Brother Limited', 10, 30)
  doc.setFontSize(15)
  ...
  */
  ...
  for (i = 0; i < event.data.length; i++) {
    ...
    if (item.notes != '') {
      let tmpi = 0;
      for (tmpi = 0; tmpi < item.notes.length; tmpi++) {
        let cc = item.notes.charCodeAt(tmpi);
        if (cc > 128) {
          item.notes = 'Unsupported character(s) found.';
          break;
        }
      }
    }
  ...
  for (i = 0; i < event.data.length; i++) {
    ...
    if (pos == -1) { // truck
      if (item.goods != 'Digger Transfer') truck.time += item.minutes;
      if (item.goods != 'Travel Time' && item.goods != 'Digger Transfer') truck.loads++;
      let j = 0;
